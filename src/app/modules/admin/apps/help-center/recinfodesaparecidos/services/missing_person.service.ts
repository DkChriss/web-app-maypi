...
export class MissingPersonService {
    ...
    private _initializeMockData(): void {
        const categories: MissingPersonCategory[] = [
            { id: uuidv4(), slug: 'informacion', title: 'Información' },
            { id: uuidv4(), slug: 'testigo', title: 'Testigo' },
            { id: uuidv4(), slug: 'avistamiento', title: 'Avistamiento' }
        ];
        ...
        for (let i = 1; i <= 50; i++) {
            ...
            missingPersons.push({
                ...
                name: `Informante ${i}`,
                email: `informante${i}@example.com`,
                phone: `+569${Math.floor(10000000 + Math.random() * 90000000)}`,
                location: location,
                latitude: latitude,
                longitude: longitude,
                date: new Date(),
                description: `Descripción detallada de la información recibida número ${i}`,
                consent: true,
                profile_image: '',
                event_image: '',
